...
export async function POST(req:any) {
    ...
    try {
      ...
      const userBlogs =  new Blog({
        title,content,author:user._id,author_name:user.fullName
      })

      const saveData = await userBlogs.save()
  
      return NextResponse.json({ message: 'User blogs add successfully', blogs: userBlogs,status:200 });
    } catch (error:any) {
